...
/**
 * Tests against LZ4 frame-compressed binary format for HTTP messages.
 */
public class LZ4FrameHttpMessageTest {

    public final String FILE = "./target/message-v31.blk.lz4f";
    ...
    public void roundTripTest() throws Exception {
        try (FileOutputStream fo = new FileOutputStream(FILE)) {
            try (LZ4FrameOutputStream bos = new LZ4FrameOutputStream(fo)) {
        ...
        try (FileInputStream fi = new FileInputStream(FILE)) {
            try (LZ4FrameInputStream bis = new LZ4FrameInputStream(fi)) {
                BinaryHttpMessage m = new BinaryHttpMessage();
                m.read(bis);
